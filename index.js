...
app.all('/checar-estoque', async (req, res) => {
    try {
        ...
        // Filtra linhas com STATUS "DISPONIVEL"
        const disponiveis = rows.filter(row => row[2] === 'DISPONIVEL');
        const count = disponiveis.length;

        // Formato esperado pelo Ease Bot conforme GitBook
        return res.json({
            status: count > 0 ? "continue" : "error",
            stock_count: count,
            reason: count > 0 ? null : "Estoque esgotado!"
        });
    } catch (error) {
...
app.post('/obter-key', async (req, res) => {
    // O Ease Bot envia os dados no corpo da requisição (POST)
    ...
    try {
        ...
        // Localiza as keys disponíveis
        for (let i = 0; i < rows.length; i++) {
            if (rows[i][2] === 'DISPONIVEL') {
        ...
        if (disponiveisIndexes.length < quantidade) {
            return res.json({ status: "error", reason: "Estoque insuficiente para a quantidade solicitada" });
        }
